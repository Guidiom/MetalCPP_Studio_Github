...
- (CameraData)uniforms               { if (_uniformsDirty) [self updateUniforms]; return _uniforms;}

// For all the setter functions, we update the instance variables and set the dirty flag
- (void)setNearPlane:(float)newNearPlane     { _nearPlane    = newNearPlane;                     _uniformsDirty = true; }
- (void)setFarPlane:(float)newFarPlane       { _farPlane     = newFarPlane;                      _uniformsDirty = true; }
- (void)setAspectRatio:(float)newAspectRatio { _aspectRatio  = newAspectRatio;                   _uniformsDirty = true; }
- (void)setViewAngle:(float)newAngle         { assert(_width == 0); _viewAngle = newAngle;       _uniformsDirty = true; }
- (void)setWidth:(float)newWidth             { assert(_viewAngle == 0); _width = newWidth;       _uniformsDirty = true; }
- (void)setPosition:(float3)newPosition      { _position     = newPosition;                      _uniformsDirty = true; }
- (void)setUp:(float3)newUp                  { [self orthogonalizeFromNewUp: newUp];             _uniformsDirty = true; }
- (void)setDirection:(float3)newDirection    { [self orthogonalizeFromNewForward: newDirection]; _uniformsDirty = true; }

- (float4x4)ViewMatrix                     { return _uniforms.viewMatrix; }
- (float4x4)ProjectionMatrix               { return _uniforms.projectionMatrix; }
- (float4x4)ViewProjectionMatrix           { return _uniforms.viewProjectionMatrix; }
- (float4x4)InvOrientationProjectionMatrix { return _uniforms.invOrientationProjectionMatrix; }
- (float4x4)InvViewProjectionMatrix        { return _uniforms.invViewProjectionMatrix; }
- (float4x4)InvProjectionMatrix            { return _uniforms.invProjectionMatrix; }
- (float4x4)InvViewMatrix                  { return _uniforms.invViewMatrix; }
